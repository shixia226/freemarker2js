module.exports = {
    ...
    */
    ...
    'list': function(template, _, engine) {
        ...
        var matchRepeat = dataLabel.match(reg_list_num);
        if (matchRepeat) { //解析 <#list 1..item.count as idx>
            jsString.push('var arr = []; for (var i = ', (reg_num.test(matchRepeat[1]) ? '' : 'context.') + matchRepeat[1], ', len = ', (reg_num.test(matchRepeat[2]) ? '' : 'context.') + matchRepeat[2], '; i <= len; i++) { arr.push(i); }');
        } else {
