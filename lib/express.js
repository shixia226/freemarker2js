module.exports = {
    ...
    */
    ...
    'list': function(template, _, engine) {
        ...
        let match = reg_list.exec(template);
        ...
        let matchRepeat = reg_list_num.exec(dataLabel);
    ...
    'if': function(template, _, engine) {
        ...
        let match = reg_if.exec(template);
...
let reg_num = /^\d+$/,
    reg_assign = /<#assign[\s]+([^\/>]+)[\s]*\/?>/g,
    reg_assign_exp = /\s*([^=\s]+)\s*=\s*([^=]+?(?:(?:>=|<=|==)[^=]+?)*)(?=(?:\s+\w+\s*=)|$)/g,
    reg_list = /(?:<#list +([\w\.\[\]]+) +as +(\w+) *>)|(<\/#list>)/g,
    reg_list_num = /(.*[^.])+\.\.([^.].*)/,
    reg_if = /(?:<#if ([^>]+)>)|(?:<#elseif ([^>]+)>)|(<#else>)|(<\/#if>)/g;